...
const buildPerformanceDashboard = (profiles, attendanceRecords, gradeRecords, user) => {
  ...
  const gradedStudents = studentSnapshots.filter((entry) => entry.averageGrade !== null);
  const summary = {
    studentsTracked: studentSnapshots.length,
    groupsTracked: groupPerformance.length,
    averageGrade: gradedStudents.length > 0
      ? Math.round(gradedStudents.reduce((sum, entry) => sum + entry.averageGrade, 0) / gradedStudents.length)
      : null,
...
async function getOpsAudit({ entityType, limit }) {
...
async function getRiskFlags({ user, from: rawFrom, to: rawTo, limit: rawLimit }) {
  ...
  const studentIdFilter = buildInClause(studentIds);
  const attendanceParams = [from, to, ...studentIdFilter.params];
  let attendanceSql = `
    SELECT
      a.student_id,
  ...
  if (canManageAcademicRecords(user) && !hasAdminAccess(user)) {
    ...
    `;
    attendanceParams.push(user.id, user.name, user.email);
  }
...
async function getPerformanceDashboard({ user, from: rawFrom, to: rawTo }) {
  ...
  return {
    from,
    to,
    ...buildPerformanceDashboard(profiles, attendanceRecords, gradeRecords, user)
  };
}

async function getFacultyOverviewReport({ user, from: rawFrom, to: rawTo }) {
  const defaultRange = getDefaultPerformanceRange();
  const from = rawFrom ? normalizeDateInput(rawFrom) : defaultRange.from;
  const to = rawTo ? normalizeDateInput(rawTo) : defaultRange.to;
  assertRange(from, to);

  const dataset = await buildAnalyticsDataset({ user, from, to });
...
async function getDeanOfficeReport({ user, from: rawFrom, to: rawTo }) {
  const defaultRange = getDefaultPerformanceRange();
  const from = rawFrom ? normalizeDateInput(rawFrom) : defaultRange.from;
  const to = rawTo ? normalizeDateInput(rawTo) : defaultRange.to;
  assertRange(from, to);
...
async function markAllNotificationsRead(userId) {
  await db.run(
    `UPDATE notification_inbox
     ...
     WHERE user_id = ?`,
    [db.client === 'postgres' ? true : 1, userId]
  );
  ...
  const unreadRow = await db.get(
    `SELECT COUNT(*) AS unread_count
     ...
     WHERE user_id = ?
       AND is_read = ?`,
    [userId, db.client === 'postgres' ? false : 0]
  );
...
async function markNotificationRead({ userId, notificationId: rawNotificationId }) {
  const notificationId = Number(rawNotificationId);
  if (!Number.isInteger(notificationId) || notificationId <= 0) {
    throw createServiceError(400, 'Notification id is invalid');
  }
  ...
  await db.run(
    `UPDATE notification_inbox
     ...
     WHERE id = ?
       AND user_id = ?`,
    [db.client === 'postgres' ? true : 1, notificationId, userId]
  );
